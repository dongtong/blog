...
var riot = require('riot');
module.exports = riot.tag2('word-counter', '<h3>{title}</h3> <input id="input" onkeyup="{increase}"> <button id="remove" onclick="{decrease}">删除</button> <div>数量: {count}</div> <div each="{languages}">{name}</div> <div each="{lang, index in languages}">{index} : {lang.name}</div>', '', '', function(opts) {
title = '文字计数器(不支持中文 :()';
count = 0;
